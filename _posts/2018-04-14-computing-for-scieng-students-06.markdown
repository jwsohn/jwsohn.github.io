---
layout: post
title:  "이공계 대학생/대학원생을 위한 컴퓨팅 강좌 6: 코딩에 도움이 되는 습관"
...
일단 이 정도까지는 다들 잘 하십니다. 들여쓰기(indentation)와 코드 블록(code block)
맞추기지요.
...
그런데 이 다음부터 다들 제각각이 되십니다. 변수 (variable)에 이름을 줘 볼까요?
이 때 코딩 스타일에 따르면 Java는 lowerCamelCase 형식을 따르고 Python은
snake_case를 따라야 합니다.
...
Python에서 `helloWorldMessage` 이름의 변수를 써도 컴파일이나 프로그램 수행에는
아무런 지장이 없습니다. `Helloworldmessage`니 `HELLO` 같은 이름을 마음대로
줘도 마찬가지입니다.

하지만, 여기서 Java의 경우는 lowerCamelCase로 변수명을 쓰기로 **개발자들간에**
관습(practice)을 만들었고 Python의 경우는 snake_case로 단어 사이에 `_`를 넣어서
변수명을 쓰기로 했다는 점을 중요하게 보셔야 하겠습니다. 그러니까, 개발자들은
이 관습을 따라야 하는 것입니다.
...
넣은 것도 주의 깊게 보셔야 합니다. 그러니까 `helloWorldMessage="Hello,
...
이것 말고도 코딩 스타일에 대해서는 신경써야 할 것들이 많습니다만 일단 간단히
여기서 줄이구요. 다시 한번 좋은 코딩 스타일 습관은 **다른 사람도 읽기 좋은**
코드를 만드는데 필수불가결이라는 사실을 꼭 기억하시기 바랍니다. 또한, **다른
사람이 짠 코드와 내 코드를 섞을 때** 같은 코딩 스타일을 쓰는 것이 얼마나
생산성을 향상시키는 가도 나중에 꼭 체크해 보시기 바랍니다.
...
통합환경은 직관적인 사용자 인터페이스에 컴파일, 디버깅 작업을 자동으로 처리해
주는 까닭에 코딩 초보자에게 좋을 것 같지만 제가 보기에는 프로그램 개발 과정을
편리함으로 지나치게 추상화 시키는 까닭에 초보자에게는 그다지 좋지 않다고
생각합니다. 또한, 초보자가 쓰기에는 기능이 많기도 합니다.
...
필요하기 때문입니다. 특히, 처음에는 컴파일 작업을 손으로 하면서 어떤 파일을
컴파일 시키면 어떤 형태의 실행 파일이 나오는지 알아야 하겠습니다.

또한, 리눅스 서버 관리자 권한이 있는 경우 어플리케이션을 설치하는 것과 같은
관리자 권한이 필요한 명령들을 자주 실습해 볼 필요가 있습니다. 그러기 위해서는
리눅스 명령 프롬프트를 많이 쓰는 환경이 필요하지요. 
...
바로바로 실행시켜볼 수 있는 인터프리터(interpreter) 역시 중요하겠지요.
기본으로 제공되는 `python` 인터프리터보다는 `ipython` 인터프리터를 쓰시길
추천합니다. `ipython`에서는 명령어에 색깔을 입혀주는 syntax highlighting, 탭
키를 누르면 가장 가까운 명령어나 변수 이름을 알아서 추측해주는 name completion
기능이 기본적으로 제공됩니다. 아래 화면을 보시지요.

![ipython](/assets/2018-04-14-computing-for-scieng-students-06/ipython.png)

마지막으로 `vim`이나 `emacs`에 아무래도 적응이 안되시는 분들은 제가 과거
포스팅에서 잠깐 언급했던 `nano` 에디터를 쓰시길 추천합니다. 그리고, Eclipse와
같은 통합환경은 개발하는 프로젝트의 크기가 커지면서 소스 코드 파일의 숫자가
많아지면 그때부터 쓰셔도 되겠습니다. 변수나 클래스, 모듈 이름을 하나만 고치면
소스코드 전체에 걸쳐서 자동으로 고쳐주는 refactoring과 같은 기능은 통합환경을
써야 편리하게 이용할 수 있으니까요.
...
하지만, 사람은 망각의 동물입니다. 내가 쓴 코드도 잘 활용하려면 comment를 잘
...
습관과 개발 문서 작성과 같이 코딩을 도와주는 습관으로 나눌 수 있겠습니다.
