...
코딩에 도움이 되는 다른 여러가지들을 종합해 보겠습니다. 그런데 이번에는 마침
글을 쓰다보니 여러가지 대학 교재에 대한 추천이 많이 포함되었습니다.
...
중요해졌는데요. 여기에 소위 빅 데이터라고 불리는 경향은 분석이 필요한 데이터의
사이즈가 너무 커져서 일반적인 통계 기법 만으로는 분석이 쉽지 않아졌다는 점에
주목하고 있습니다. 또한, 데이터의 크기가 커지고 컴퓨팅 파워가 증대되면서
기존에는 유용하지 않았던 기법이 효율이 높아지는 경우도 발생하고 있습니다.
...
Strang의 홈페이지에 가 보면 교재에서 중요한 부분들을 발췌해 두었습니다. 행렬의
소개, 역행렬 연산, Eigenvalue, image processing, 평균 분산 및 확률 등등인데요.
